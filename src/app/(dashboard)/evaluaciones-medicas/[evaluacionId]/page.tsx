...
export default async function Page({
  ...
  const evaluacion = await consultarEvaluacion(u.id, evaluacionId);
  if (!evaluacion) notFound();
  const [contexto, recetaExistenteId] = await Promise.all([
    consultarContextoEvaluacion(u.id, evaluacion.trabajadorId),
    evaluacion.estado === "FINALIZADA"
      ? consultarRecetaPorEvaluacion(u.id, evaluacionId)
      : null,
  ]);
  return (
    <div className="space-y-6">
      <Migas />
      <EncabezadoPagina
        titulo={evaluacion.numeroEvaluacion ?? "Evaluación médica"}
